...
class Link{
	private:
		...
		bool noblock_;
		bool error_;
		std::vector<Bytes> recv_data; //输入到数据

		RedisLink *redis; //redisLink
	public:
		...
		static Link* connect(const char *ip, int port); //创建连接的静态函数
		static Link* listen(const char *ip, int port);  // 创建监听函数
		Link* accept();   //接受连接
