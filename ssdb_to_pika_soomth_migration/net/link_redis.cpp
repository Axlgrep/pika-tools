...
static std::map<std::string, RedisRequestDesc> cmd_table; //命令表  cmd到请求协议描述映射

struct RedisCommand_raw  //原始redis请求描述
{
	int strategy;
...
static RedisCommand_raw cmds_raw[] = {  //原始请求数组
...
int RedisLink::convert_req(){
	if(!inited){
		...
		while(def->redis_cmd != NULL){
			...
			cmd_table[desc.redis_cmd] = desc;  //redis__cmd 到请求描述的映射
			def += 1;
		}
	}
	...
	it = cmd_table.find(cmd);  //得到命令在map中查
	if(it == cmd_table.end()){  //如果表中没有查到
		recv_string.push_back(cmd);  //把命令加入string数组里
		for(int i=1; i<recv_bytes.size(); i++){
			recv_string.push_back(recv_bytes[i].String()); //把字节数组变成String
		}
		return 0;
	}
...
//Buffer是缓冲区，不是自己掌握内存，所以先把Buffer中内容，进行分割，返回byte数组，因为Byte数组都是
//指针，不具有实际数据， 再讲byte里的数据（实际数据还在buffer中，用byte切割好）
//讲byte分割好的内存，转为string，存储到recv_string中，在让recv_byte中的指针指向recv_string中
//感觉...绕
const std::vector<Bytes>* RedisLink::recv_req(Buffer *input){
	...
	cmd = recv_bytes[0].String(); //第一个bytes 是命令
	strtolower(&cmd);
...
int RedisLink::parse_req(Buffer *input){
	...
	while(size > 0){
		...
		recv_bytes.push_back(Bytes(ptr, len));  //把网络协议，按照byte存起来,讲命令切割
