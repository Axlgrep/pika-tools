...
int SSDBImpl::qget_front(const Bytes &name, uint64_t* ret) {
        qget_uint64(this->ldb, name, QFRONT_SEQ, ret);
}
...
int64_t SSDBImpl::_qpush(const Bytes &name, const Bytes &item, uint64_t front_or_back_seq, char log_type){
	...
	if(seq <= QITEM_MIN_SEQ || seq >= QITEM_MAX_SEQ){
        ...
        uint64_t debug_seq;
        ret = qget_uint64(this->ldb, name, front_or_back_seq, &debug_seq);
